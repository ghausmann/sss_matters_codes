function [ncoeffs,model,T,J,iter]=tpsolveMS2(coeffs,x0,model,params,msparams,transition,c0,nep,P,tolX,tolF,maxiter,varargin)
...
while normd > max(tolX,eps) && normT>max(tolF,eps) && iter<maxiter 
    ...
    [T,J]=tpscaleMS(weighted_coeffs,x0,model,params,msparams,transition,c0,nep,P,Wcols,Wrows);  
...
if logical(normd < max(tolX,eps) || normT<tolF)==0
    ...
    if isa(OPTIONS,'optim.options.Fsolve')
        ...
        [ncoeffs,T,exitflag,output,J]=fsolve(@(coeffs) tpscale_nls(coeffs,x0,model,params,msparams,transition,c0,nep,P,Wcols,Wrows),weighted_coeffs,OPTIONS);
    elseif isa(OPTIONS,'optim.options.Lsqnonlin')
        fprintf(2,'Could not find an exact solution, switching to lsqnonlin ...\n');
        [ncoeffs,~,T,exitflag,output,~,J]=lsqnonlin(@(coeffs) tpscaleMS(coeffs,x0,model,params,msparams,transition,c0,nep,P,Wcols,Wrows),weighted_coeffs,[],[],OPTIONS);
    else
...
ncoeffs=reshape(full(coeffs),(model.n_y+model.n_x1)*model.n_b,model.n_regimes);
