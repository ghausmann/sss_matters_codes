...
% Computes IRFs (for figures 3, 4, 5, 6, 7, 9, 10, 13, and 14)
%---------------------------------------------------------------------------
...
Brazil = 0;
Ecuador = 0;
%set the calibration type: 0 is NX (default), 1 is NFA (only for Argentina)
calibration_type = 0;
...
if Argentina ==1
    ...
    if origin_est==1
         ...
         if calibration_type==0
             ...
             load('my_est_params_pac_sss_nfa');
         end
        %load('my_est_params_pac_sss_argentina_5o'); %load estimated parameters
...
elseif Brazil ==1
    ...
    r_bar=(0.007);
    if origin_est==1
        load('my_est_params_pac_sss_brazil'); %load estimated parameters
        phi_d = my_est_params(1); %controls Uzawa
        phipar = my_est_params(2); %controls capital adjustment costs
        sigma_x = my_est_params(3); %std. of innovations to productivity
        D1 = my_est_params(4); %SSS debt
...
elseif Ecuador == 1
    ...
    if origin_est==1
        load('my_est_params_pac_sss_ecuador'); %load estimated parameters
        phi_d = my_est_params(1); %controls Uzawa
        phipar = my_est_params(2); %controls capital adjustment costs
        sigma_x = my_est_params(3); %std. of innovations to productivity
        D1 = my_est_params(4); %SSS debt
