...
% Estimation of parameters with a larger debt target (Table 5 in the paper)
%---------------------------------------------------------------------------
...
load('my_est_params_pac_sss_nfa.mat');
%set target for moments from the data, and initial guesses for estimated parameters
target = [4.77;1.31;3.81;16.90]; %Target moments from Argentina data
...
myfun=@(x)my_smm_diff_function_sssp_nfa(x,target,replications,innovations,model,params,my_dss_params,M,eps_ind,approx0,approx1,Ti,1);
...
yf = my_smm_moments_sss_comparep_nfa(my_est,replications,innovations,model,params,my_dss_params,M,eps_ind,approx0,approx1,Ti,1)

%save the results
my_est_params = [phi_d phipar sigma_x D1]

if save_estimate==1
    save_path = fullfile(my_root,'estimated parameters\my_est_params_pac_sss_nfa.mat');
