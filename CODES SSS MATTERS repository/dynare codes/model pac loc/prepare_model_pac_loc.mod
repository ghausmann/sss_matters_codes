 ...
 *  Auxiliary perturbation model described in Appendix B of the paper
 */
...
// Auxiliary parameters for two-parameter perturbation
psi_k = 0;
psi_i = 0;
Da = D_bar;
psi_d = 4e-4;
...
%Eq. 2.25 in paper:
exp(K) = (1-(delta+psi_i*(1-eps^2)))*exp(K(-1)) + (1-phipar/2*(exp(I)/exp(I(-1))-1)^2)*exp(I);
%Eq. 2.3 in paper:
...
%Eq. 2.5 in paper:
(exp(C))^(-nu)=exp(lambda);
%Modified Euler Eq. for debt (Appendix B) 
exp(lambda)*(1/(1+ r) - (eps^2)*phi_d*(D-D_bar) - (1-eps^2)*psi_d*(D-Da) ) = betta*exp(lambda(+1));
%Eq. 2.22 in paper:
exp(phi) = betta*( ( 1-( delta+psi_k*(1-eps^2) ) )*exp(phi(+1))+(alppha)*(exp(Y(+1))/exp(K))*exp(lambda(+1)));
%Eq. 2.8 in paper:
