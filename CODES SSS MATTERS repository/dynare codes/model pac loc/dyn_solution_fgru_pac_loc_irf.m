...
% Computes IRFs (for Figure 2 in paper, excluding Euler errors)
%---------------------------------------------------------------------------
...
psi_d=4e-4;
M_.params(strcmp(M_.param_names,'psi_d')==1)  = psi_d;
...
%Solve for the SSS
tic
sss_sol = compute_sss_fgru_3_locp_num_dyn(M_,options_,oo_,my_dss_params,my_ind,nshocks,[1 log(Kstar0) 0])
toc

%Recompute DSS of auxiliary model
[yss,psi_k] = my_num_dss(sss_sol,my_dss_params);
M_.params(strcmp(M_.param_names,'Da')==1)    = sss_sol(1);
M_.params(strcmp(M_.param_names,'psi_k')==1) = psi_k;
M_.params(strcmp(M_.param_names,'psi_i')==1) = sss_sol(3);
...
%This will be the starting point of the Economy (dr_yt is a function from my RED paper package)
ysss0 = dr_yt(mdr,yss,2,x0-yss(st_ini_ind:st_end_ind,1),zeros(nshocks,1));
...
idx0 = find(strcmp(oo_.var_list,'C'));lCt = myt(oo_.dr.order_var == idx0,:);c_sss = exp(ysss0(oo_.dr.order_var == idx0));
idx0 = find(strcmp(oo_.var_list,'Y'));lYt = myt(oo_.dr.order_var == idx0,:);y_sss = exp(ysss0(oo_.dr.order_var == idx0));
idx0 = find(strcmp(oo_.var_list,'Dp'));Dpt = myt(oo_.dr.order_var == idx0,:);D_sss = (ysss0(oo_.dr.order_var == idx0));
idx0 = find(strcmp(oo_.var_list,'Ip'));lIpt = myt(oo_.dr.order_var == idx0,:);i_sss = exp(ysss0(oo_.dr.order_var == idx0));
